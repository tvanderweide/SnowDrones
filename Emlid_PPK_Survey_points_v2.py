...
looped for Validation Targets (in progress)
"""
...
def mkGPSProcessDirs(save_fold, csvFN):
    dateFN = csvFN.rpartition("/")[2].rpartition("_")[0]
    # Make the GPSProcess Folder Directories
    savePath = save_fold + dateFN + "/"
    saveBasePath = savePath + "Base/"
    saveRoverPath = savePath + "Rover/"
    directory = os.path.dirname(savePath)
    if not os.path.exists(directory):
        os.makedirs(directory)
        if not os.path.exists(saveBasePath):
            os.mkdir(saveBasePath)
        if not os.path.exists(saveRoverPath):
            os.mkdir(saveRoverPath)

def process():
    #Define CSV and POS file locations
    main_fold = 'C:/Users/Endure1/Documents/SnowEx/GPSData/2021-08-04/'
    csv_fold = main_fold + "Rover/Rover_CSV/"
    save_fold = 'C:/Users/Endure1/Documents/SnowEx/GPSData/GPSProcessing/'
    
    # Process all the dates
    for csvFN in sorted(glob.iglob(csv_fold + "*.csv")):
        csvFN = csvFN.replace('\\', '/')
        dateFN = csvFN.rpartition("/")[2].rpartition("_")[0]
        
        # mkGPSProcessDirs(save_fold, csvFN)
        
    for ubxfile in sorted(glob.iglob(base_fold + ".ubx")):
        print(ubxfile)
        ...
        #Write out new file
        out_fn = main_fold + csv_fn + 'orig_ppk_pos.csv'
